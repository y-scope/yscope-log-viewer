...
import {
    isJsonObject,
} from "../../decoders/JsonlDecoder/utils";
...
const getFormattedField = (
    ...
    let fields: JsonValue | undefined = logEvent.fields;
    if (null !== mergedKvPairKeys) {
        let mergedKvPairKey = fieldPlaceholder.parsedFieldName.isAutoGenerated ?
            mergedKvPairKeys.autoGeneratedKey :
            mergedKvPairKeys.userGeneratedKey;

        fields = fields[mergedKvPairKey];
        if ("undefined" != typeof fields && false === isJsonObject(fields)) {
 ...
 */
const splitFieldPlaceholder = (placeholderString: string, mergedKvPairKeys: Nullable<MergedKvPairKeys>): {
    ...
    const parsedFieldName: ParsedFieldName = parseFieldName(fieldName);
    if (null === mergedKvPairKeys && parsedFieldName.isAutoGenerated) {
        throw new Error(
