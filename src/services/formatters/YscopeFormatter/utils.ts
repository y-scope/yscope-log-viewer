...
const getLogEventFields = (logEvent: LogEvent, structuredIrNamespaceKeys: Nullable<StructuredIrNamespaceKeys>, isAutoGenerated: boolean): JsonObject => {
    if (null === structuredIrNamespaceKeys) {
        return logEvent.fields
    }
    const namespaceKey = isAutoGenerated ?
...
const getFormattedField = (
    ...
    if (null === structuredIrNamespaceKeys) {
        ...
        fields = getLogEventFields(
            logEvent,
            structuredIrNamespaceKeys,
            fieldPlaceholder.parsedFieldName.isAutoGenerated);
    }
