...
import {Formatter, ParsedFieldName, MergedKvPairKeys} from "../../typings/formatters";
...
type StructuredIrReaderOptions = {
...
class ClpIrDecoder implements Decoder {
    ...
    constructor (
        ...
        streamReader: ClpStreamReader,
        decoderOptions: DecoderOptions,
        mergedKvPairKeys: MergedKvPairKeys
    ) {
        this.#streamType = streamType;
        this.#streamReader = streamReader;
        if (streamType === CLP_IR_STREAM_TYPE.STRUCTURED) {
            this.#formatter = new YscopeFormatter({formatString: decoderOptions.formatString, mergedKvPairKeys});
    ...
    static async create (
        ...
        const mergedKvPairKeys: MergedKvPairKeys = {
            module.MERGED_KV_PAIRS_AUTO_GENERATED_KEY,
            module.MERGED_KV_PAIRS_USER_GENERATED_KEY
        };

        return new ClpIrDecoder(streamType, streamReader, decoderOptions, mergedKvPairKeys);
    }
